# Documentation

Start here for project context, then work through the docs below in order.

| Doc | Description |
|-----|-------------|
| [vision.md](vision.md) | Why the project exists; strategy memo for Columbia County Democrats |
| [architecture.md](architecture.md) | System design, guiding principles, deployment model |
| [data-model.md](data-model.md) | CiviCRM entities, relationships, and CoCoDems-specific modeling |
| [roadmap.md](roadmap.md) | Phased milestones from foundation through production |
| [deployment.md](deployment.md) | Staging deploy runbook (bootstrap, Docker, TLS) |
| [dns.md](dns.md) | DNS configuration for staging and production |
| [adr/](adr/) | Architecture Decision Records — why key technical choices were made |
| [docker/README.md](../docker/README.md) | Local development environment setup and troubleshooting |

**Also useful:** [coding-standards.md](coding-standards.md) — conventions for PHP, JavaScript, Terraform, Docker, Git, and documentation.

For a quick local start, see [Getting started](../README.md#getting-started) in the repository root README.
